...
def validate_workload_layout(context: ValidationContext) -> list[str]:
    ...
    for app_name, (service, logical_image) in WORKLOAD_APPLICATIONS.items():
        ...
        if source != expected:
            errors.append(
                f"{app_name} source must be {expected}, found {source or 'missing'}"
            )
        images: list[str] = []
        for resource in context.render(f"k8s/apps/base/{service}"):
            pod_spec = ((resource.get("spec") or {}).get("template") or {}).get(
                "spec"
            ) or {}
        ...
        if not images:
            errors.append(
                f"{service} base does not reference logical image {logical_image}"
            )
        errors.extend(
...
def validate_lifecycle(context: ValidationContext) -> list[str]:
    ...
    for application in applications:
        ...
        if not {entry.get("name") for entry in spec.get("info") or []} >= {
            "Owner",
            "Source",
            "Runbook",
        }:
        ...
        if any(source.get("chart") for source in sources):
            ...
            if not (
                name.startswith(expected[0])
                and project == expected[1]
                and plane == expected[2]
            ):
                errors.append(
                    f"Helm Application {name} must belong to the addons lifecycle boundary"
                )
        else:
            ...
            boundary = next(
                (
                    contract
                    for prefix, contract in boundaries.items()
            ...
            if not boundary:
                errors.append(
                    f"Git Application {name} has unsupported lifecycle source paths: {paths}"
                )
            elif not (
                name.startswith(boundary[0])
                and project == boundary[1]
                and plane == boundary[2]
            ):
                errors.append(f"Git Application {name} violates its lifecycle boundary")
        expected_wave = expected_waves.get(name)
        if (
            expected_wave
            and (metadata.get("annotations") or {}).get("argocd.argoproj.io/sync-wave")
            != expected_wave
        ):
...
def validate_projects_and_cluster_configuration(
    ...
    if any(resource.get("kind") == "Namespace" for resource in root_resources):
        errors.append(
            "root GitOps must own only control-tree resources, never Namespace resources"
        )
    for project in (
        "mlops-cluster",
        "mlops-platform",
    ...
    if cluster_kinds != expected_kinds:
        errors.append(
            "mlops-cluster must allow only declared cluster-configuration resource kinds"
        )
    if (projects["mlops-addons"].get("spec") or {}).get("clusterResourceWhitelist") != [
        {"group": "*", "kind": "*"}
    ]:
        errors.append(
            "mlops-addons must retain trusted add-on chart cluster-resource allowance"
        )
    ...
    if namespaces != PRODUCTION_NAMESPACES:
        errors.append(
            f"cluster namespace set must be {sorted(PRODUCTION_NAMESPACES)}, found {sorted(namespaces)}"
        )
    namespace_app = context.application("mlops-prod-cluster-namespaces")
    if (
        ((namespace_app.get("spec") or {}).get("source") or {}).get("path")
    ) != "k8s/cluster/namespaces":
        errors.append(
            "mlops-prod-cluster-namespaces must source k8s/cluster/namespaces"
        )
    ...
    if storage_classes != {"ebs-gp3"}:
        errors.append(
            "mlops-prod-cluster-storage must own exactly the ebs-gp3 StorageClass"
        )
    return errors
...
def validate_no_default_namespace(context: ValidationContext) -> list[str]:
    ...
    for manifest_path in sorted([*k8s_root.rglob("*.yaml"), *k8s_root.rglob("*.yml")]):
        ...
        if ".default.svc.cluster.local" in content:
            errors.append(
                f"{relative_path} must not reference a Service in default.svc.cluster.local"
            )
        for document in yaml_documents(content):
            for key, value in _walk_values(document):
                if key == "namespace" and value == "default":
                    errors.append(
                        f"{relative_path} must not declare namespace: default"
                    )
                    break
    return errors
