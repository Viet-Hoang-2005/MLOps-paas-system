...
def validate(context: ValidationContext) -> list[str]:
    ...
    if (
        (source_spec.get("template") or {}).get("serviceAccountName")
    ) != "argo-eventsource-sa":
        errors.append(
            "EventSource must use the non-privileged dedicated service account"
        )
    ...
    if (
        (((event_bus.get("spec") or {}).get("nats") or {}).get("native") or {}).get(
            "auth"
        )
    ) != "token":
    ...
    if "body.namespace" in rendered_text:
        errors.append(
            "training namespace must be fixed in Git, not accepted from an event payload"
        )
    ...
    for role in (resource for resource in execution if resource.get("kind") == "Role"):
        for rule in role.get("rules") or []:
            if set(rule.get("resources") or []) & forbidden:
                errors.append(
                    f"Role {(role.get('metadata') or {}).get('name')} grants forbidden execution access"
                )
    ...
    for trigger in (sensor.get("spec") or {}).get("triggers") or []:
        workflow = (
            (
                ((trigger.get("template") or {}).get("k8s") or {}).get("source") or {}
            ).get("resource")
        ) or {}
        ...
        if not workflow_spec.get("workflowTemplateRef"):
            errors.append(
                "every Sensor trigger must reference a Git-managed WorkflowTemplate"
            )
        if workflow_spec.get("serviceAccountName"):
            errors.append(
                "Sensor trigger must not override a WorkflowTemplate service account"
            )
    policies = {
    ...
    required_policies = {
        "allow-celery-worker-to-argo-events-webhook",
        "isolate-argo-events-eventbus",
    }
    if not required_policies <= policies:
        errors.append(
            "execution Application must render EventSource and EventBus NetworkPolicies"
        )
    webhook_policy = find_resource(
        execution, "NetworkPolicy", "allow-celery-worker-to-argo-events-webhook"
    )
    if ((webhook_policy.get("spec") or {}).get("ingress") or []) != [
            ...
            "from": [
                {
                    "namespaceSelector": {
                        "matchLabels": {
                            "kubernetes.io/metadata.name": "mlops-control-plane"
                        }
                    },
                    "podSelector": {"matchLabels": {"app": "mlops-paas-celery-worker"}},
                }
            ],
        ...
        errors.append(
            "EventSource NetworkPolicy must allow only the mlops-control-plane worker on TCP 12000"
        )
    targets = {
        (
    ...
    if ("argo-events", "argo-events-webhook-server") not in targets:
        errors.append(
            "Argo Events server bearer-token Secret must be synchronized by External Secrets"
        )
    for deployment_name, expected_secret in {
        "mlops-paas-control-plane": "control-plane-api-secret",
        "mlops-paas-celery-worker": "celery-worker-secret",
    }.items():
        deployment = find_resource(control_plane, "Deployment", deployment_name)
        pod_spec = ((deployment.get("spec") or {}).get("template") or {}).get(
            "spec"
        ) or {}
        if pod_spec.get("automountServiceAccountToken") is not False or pod_spec.get(
            "serviceAccountName"
        ):
            errors.append(
                f"{deployment_name} must not receive Kubernetes API credentials"
            )
        token_env = [
            env
            for container in [
        ...
        if not token_env or any(
            ...
            errors.append(
                f"{deployment_name} must source the Argo bearer token from {expected_secret}"
            )
    workflow_addon = context.application("mlops-prod-addon-argo-workflows")
    values = (
        ((workflow_addon.get("spec") or {}).get("source") or {}).get("helm") or {}
    ).get("values", "")
    restrictions = ((yaml.safe_load(values) or {}).get("controller") or {}).get(
    ...
    workflow_namespaces = ((yaml.safe_load(values) or {}).get("controller") or {}).get(
        "workflowNamespaces"
    )
