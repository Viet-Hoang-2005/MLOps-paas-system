...
def validate_policy(context: ValidationContext) -> tuple[list[str], dict]:
    ...
    if (
        source.get("repoURL") != "https://kyverno.github.io/kyverno"
        or source.get("chart") != "kyverno"
        or source.get("targetRevision") != "3.8.2"
    ):
        errors.append(
            "mlops-prod-addon-kyverno must pin the official Kyverno chart at 3.8.2"
        )
    if (
        ((kyverno.get("spec") or {}).get("destination") or {}).get("namespace")
    ) != "kyverno":
    ...
    if (
        ((verification_app.get("spec") or {}).get("source") or {}).get("path")
    ) != "k8s/cluster/policies/image-verification":
        errors.append("image-verification must use the Git-backed policy source")
    if (
        ((verification_app.get("spec") or {}).get("destination") or {}).get("namespace")
    ) != "kyverno":
    ...
    credentials = find_resource(
        resources, "ExternalSecret", "kyverno-harbor-registry-auth-sync"
    )
    credentials_spec = credentials.get("spec") or {}
    target = credentials_spec.get("target") or {}
    if (
        resource_namespace(credentials) != "kyverno"
        or target.get("name") != "kyverno-harbor-registry-auth"
    ):
        errors.append(
            "Kyverno registry credential must be scoped to kyverno/kyverno-harbor-registry-auth"
        )
    ...
    if (
        spec.get("background") is not False
        or spec.get("failurePolicy") != "Fail"
        or spec.get("validationFailureAction") != "Enforce"
    ):
        errors.append("image verification must be fail-closed and enforced")
    if spec.get("webhookTimeoutSeconds") != 30:
        errors.append(
            "image verification must use a bounded 30-second registry timeout"
        )
    verify_images = ((spec.get("rules") or [{}])[0].get("verifyImages")) or []
    if len(verify_images) != 1:
        errors.append(
            "image verification policy must contain exactly one verifyImages rule"
        )
        return errors, {}
    ...
    if (verification.get("imageRegistryCredentials") or {}).get("secrets") != [
        ...
        errors.append(
            "image verification must authenticate to Harbor with its scoped credential"
        )
...
def validate_cd_workflow(context: ValidationContext, keyless: dict) -> list[str]:
    errors: list[str] = []
    workflow = (context.source_root / ".github/workflows/cd.yml").read_text(
        encoding="utf-8"
    )
    ...
    if not re.search(
        ...
        errors.append(
            "build-and-push must grant id-token: write for keyless Cosign signing"
        )
    ...
    if (
        'cosign sign --yes "${IMAGE_REFERENCE}"' not in build
        or "${IMAGE}@${DIGEST}" not in build
    ):
    ...
    if not promotion:
        ...
        if (
            "id-token: write" in promotion
            or "aws-actions/configure-aws-credentials@" in promotion
        ):
            errors.append("gitops-promotion must not receive AWS OIDC credentials")
        if (
            "secretsmanager" in promotion.lower()
            or "Fetch Secrets from AWS" in promotion
        ):
            errors.append("gitops-promotion must not fetch AWS Secrets Manager values")
    for name in re.findall(r"^  ([A-Za-z0-9_-]+):\n", workflow, re.MULTILINE):
        if name not in {"build-and-push"} and "id-token: write" in job_block(
            workflow, name
        ):
            errors.append(
                f"only build-and-push may receive id-token: write, found it in {name}"
            )
    return errors
