...
export function ModelSelectionProvider({ children }: { children: ReactNode }) {
  ...
  const match = useMemo(() => {
    const routeMatch =
      matchPath("/dashboard/home/models/:modelId", location.pathname) ||
      matchPath("/dashboard/home/model-testing/:modelId", location.pathname) ||
      matchPath(
        "/dashboard/drift-monitoring/:modelId/new",
        location.pathname,
      ) ||
      matchPath(
        "/dashboard/drift-monitoring/:modelId/report/:runId",
        location.pathname,
      ) ||
      ...
      matchPath(
        "/dashboard/management/model/:modelId/:tab",
        location.pathname,
      ) ||
    ...
    return {
      ...
      selectModel: (modelId: string) => {
        ...
        if (match && urlModelId) {
          ...
          const base = supportedBases.find(
            (b) => location.pathname === b || location.pathname === `${b}/`,
          );
          if (base) {
  ...
  useEffect(() => {
    ...
    const baseMatch = supportedBases.find(
      (b) => location.pathname === b || location.pathname === `${b}/`,
    );
    if (baseMatch) {
      navigate(
        `${baseMatch}/${selectedModel.id}${location.search}${location.hash}`,
        { replace: true },
      );
      return;
    }
