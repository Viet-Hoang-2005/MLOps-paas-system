...
export const updateProfileAvatar = async (
  ...
  return (
    await apiClient.put<MessageResponse>("/auth/profile/", formData, {
...
export const selectProfileAvatar = async (
  avatarId: string,
): Promise<MessageResponse> =>
  (
    await apiClient.post<MessageResponse>(
      `/auth/profile/avatars/${avatarId}/select/`,
    )
  ).data;
...
export const verifyPasswordChangeOTP = async (
  ...
  (
    await apiClient.post<PasswordChangeVerifyResponse>(
      "/auth/profile/password-otp/verify/",
      { otp_code: otpCode },
    )
  ).data;
