...
import type {
  UploadModelContext,
  UploadStep,
  UploadTransitionState,
} from "@/features/deploy/uploadModelContext";
import type {
  Build,
  BuildInputForm,
  Deployment,
...
const fileIdentity = (file: File | null | undefined) =>
  file ? `${file.name}:${file.size}:${file.lastModified}` : "";
const metadataFingerprint = (form: ProjectMetadataForm) =>
  JSON.stringify({
    name: form.name.trim(),
...
const hasBuildInput = (form: BuildInputForm) =>
  Boolean(
...
export default function UploadModelPage() {
  ...
  const [metadataForm, setMetadataForm] =
    useState<ProjectMetadataForm>(emptyMetadata);
  const [buildForm, setBuildForm] = useState<BuildInputForm>(emptyBuild);
  const [build, setBuild] = useState<Build | null>(null);
  const [deployment, setDeployment] = useState<Deployment | null>(null);
  const [transitionState, setTransitionState] =
    useState<UploadTransitionState>("idle");
  const [buildChangedSinceSubmission, setBuildChangedSinceSubmission] =
    useState(false);
  ...
  const buildInputsDirty = build
    ? buildChangedSinceSubmission
    : hasBuildInput(buildForm);
  const submittedBuildFailed = build
    ? ["failed", "cancelled"].includes(build.status)
    : false;
  const hasUnsavedBrowserFiles =
    metadataDirty ||
    ...
    getModelProject(requestedModelId)
      ...
      .catch((error) => {
        toast.error(
          getApiErrorMessage(
            error,
            t("uploadFlow.messages.metadataLoadFailed"),
          ),
        );
        navigate(`${uploadPath}/metadata`, { replace: true });
      });
    ...
    Promise.all([getBuild(requestedBuildId), listDeployments()])
      .then(([result, deployments]) => {
        ...
        if (requestedModelId && result.project_id !== requestedModelId)
          throw new Error(t("uploadFlow.messages.buildOwnership"));
        setBuild(result);
        setDeployment(
          deployments.find((item) => item.build_id === result.id) ?? null,
        );
        setBuildForm((current) =>
          hasBuildInput(current)
            ? current
            : {
                ...current,
      ...
      .catch((error) => {
        toast.error(
          getApiErrorMessage(error, t("uploadFlow.messages.buildLoadFailed")),
        );
        const suffix = requestedModelId ? `?modelId=${requestedModelId}` : "";
        navigate(`${uploadPath}/build${suffix}`, { replace: true });
      });
  ...
  const setMetadataField: UploadModelContext["setMetadataField"] = (
    field,
    value,
  ) => {
  ...
  const continueFromMetadata = useCallback(async () => {
    ...
    try {
      ...
      if (!project) nextProject = await createProjectMetadata(metadataForm);
        ...
        nextProject = await updateProjectMetadata(project.id, metadataForm);
      ...
      toast.error(
        getApiErrorMessage(error, t("uploadFlow.messages.metadataSaveFailed")),
      );
    } finally {
  ...
  const startBuild = useCallback(async () => {
    ...
    try {
      ...
      toast.error(
        getApiErrorMessage(error, t("uploadFlow.messages.buildStartFailed")),
      );
    } finally {
      setTransitionState("idle");
    }
  ...
  const deploy = useCallback(async () => {
    ...
    try {
      ...
      toast.error(
        getApiErrorMessage(error, t("uploadFlow.messages.deployFailed")),
      );
    } finally {
      setTransitionState("idle");
    }
  ...
  const handleDeploymentCompleted = useCallback(
    (status: string) => {
