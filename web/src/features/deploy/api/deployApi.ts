  ...
  return new Promise<void>((resolve, reject) => {
    ...
    xhr.onload = () => {
      if (xhr.status >= 200 && xhr.status < 300) {
        ...
        reject(
          new Error(
            `Direct S3 upload failed with status ${xhr.status}: ${xhr.statusText}`,
          ),
        );
      }
    };
    ...
    xhr.onerror = () => {
      reject(
        new Error("Network error occurred during direct S3 artifact upload."),
      );
    };
...
export const startProjectBuild = async (
  ...
  if (payload.source_artifact instanceof File) {
    ...
    try {
      ...
      console.warn(
        "Direct-to-S3 upload failed, attempting fallback to multipart upload:",
        directUploadErr,
      );
    }
  }
...
export const cancelBuildById = async (buildId: string): Promise<Build> =>
  (await apiClient.post<Build>(controlPlaneURL(`/builds/${buildId}/cancel/`)))
    .data;

export const listProjectBuilds = async (modelId: string): Promise<Build[]> =>
  (await apiClient.get<Build[]>(controlPlaneURL(`/models/${modelId}/builds/`)))
    .data;
...
export const listBuilds = async (): Promise<Build[]> => {
  const { data } = await apiClient.get<{ results: Build[] } | Build[]>(
    controlPlaneURL("/builds/"),
  );
  return pageResults(data);
};
...
export const listDeployments = async (): Promise<Deployment[]> => {
  ...
  >(controlPlaneURL("/deployments/"));
  return pageResults(data);
};

export const deployModelProject = async (
  modelId: string,
): Promise<ModelProject> => {
  ...
  const { data: deployment } = await apiClient.post<Deployment>(
    controlPlaneURL("/deployments/"),
    { build: build.id },
  );
  return {
...
export const stopModelEndpoint = async (
  modelId: string,
): Promise<ModelProject> => {
  ...
  const deployment = deployments.find(
    (item) => versionIds.has(item.version_id) && item.status !== "stopped",
  );
  if (deployment)
    await apiClient.post(
      controlPlaneURL(`/deployments/${deployment.id}/stop/`),
    );
  return { ...project, status: "stopped", endpoint_status: "stopped" };
};

export const checkModelEndpointHealth = async (
  modelId: string,
): Promise<ModelProject> => {
  ...
  const versionIds = new Set(versions.map((version) => version.id));
  const endpoint = (data.results ?? []).find((item) =>
    versionIds.has(item.version_id),
  );
  return {
    ...project,
    ...
    endpoint_status:
      endpoint?.health_status === "healthy" ? "healthy" : "unhealthy",
  };
};

export const triggerModelProjectBuild = async (
  modelId: string,
): Promise<ModelProject> => {
  const [project, versions] = await Promise.all([
    getModelProject(modelId),
  ...
  const { data } = await apiClient.post<Build>(controlPlaneURL("/builds/"), {
...
export const cancelBuild = async (modelId: string): Promise<void> => {
  ...
  if (build)
    await apiClient.post(controlPlaneURL(`/builds/${build.id}/cancel/`));
};
