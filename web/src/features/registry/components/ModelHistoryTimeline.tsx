    ...
    <div className="relative border-l border-border ml-3 py-4 space-y-8">
      {history.map((event, idx) => {
        ...
        if (event.action === "registered")
          actionText = t("historyTimeline.registered");
        else if (event.action === "built")
          actionText = t("historyTimeline.built");
        else if (event.action === "deployed")
          actionText = t("historyTimeline.deployed");
        else if (event.action === "promoted")
          actionText = t("historyTimeline.promoted");
        else if (event.action === "rolled_back")
          actionText = t("historyTimeline.rolledBack", {
            version: formatVersion(event.version),
          });
        else if (event.action === "stopped")
          actionText = t("historyTimeline.stopped");
        ...
        const renderTransition = () => {
          ...
          if (event.action === "promoted" || event.action === "rolled_back") {
            const from =
              !event.from_stage || event.from_stage === "none"
                ? t("historyTimeline.noProduction")
                : formatVersion(event.from_stage);
            const to =
              event.to_stage && event.to_stage !== "production"
                ? formatVersion(event.to_stage)
                : formatVersion(event.version);

            return (
              <span className="text-style-caption text-color-muted-foreground font-medium">
                {t("historyTimeline.previousCurrent", {
                  previous: from,
                  current: to,
                })}
              </span>
          ...
          <div key={event.id || idx} className="relative pl-8">
            ...
            <div className="flex flex-col gap-1.5">
              <div className="flex items-center gap-2 flex-wrap">
                ...
                <span className="font-mono text-style-code-sm font-semibold bg-muted border border-border px-2 py-0.5 rounded-compact text-color-foreground">
                  {formatVersion(event.version)}
                </span>
                {renderTransition()}
              ...
              {event.action === "promoted" || event.action === "rolled_back" ? (
                <p className="text-style-body text-color-muted-foreground">
                  {t("historyTimeline.productionMarkerUpdated")}
                </p>
              ) : (
                <p className="text-style-body text-color-muted-foreground">
                  {event.message}
