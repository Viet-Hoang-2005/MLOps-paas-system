...
export const getRegistryVersions = async (
  familyId: string,
): Promise<RegistryVersion[]> =>
  (await getProjectVersions(familyId)).map(asRegistryVersion);

export const getRegistryVersion = async (
  versionId: string,
): Promise<RegistryVersion> =>
  asRegistryVersion(
    (
...
export const getRegistryHistory = async (
  familyId: string,
): Promise<RegistryHistory[]> => {
  const versions = await getProjectVersions(familyId);
  return versions.flatMap((version) => {
    const events =
      (
        version as ModelVersion & {
          events?: Array<{
            id: string;
            event_type: string;
            from_state: string;
...
export const compareRegistryVersions = async (
  ...
  return {
    ...
    deployability_diff: {
      left: {
        status: left.deployability_status ?? "unknown",
        reason: left.deployability_reason ?? "",
      },
      right: {
        status: right.deployability_status ?? "unknown",
        reason: right.deployability_reason ?? "",
      },
    },
    deployment_diff: {
...
export const promoteRegistryVersion = async (
  ...
  await apiClient.post(
    controlPlaneURL(`/registry/models/${familyId}/aliases/`),
    {
      name: alias,
      version: versionId,
    },
  ...
  return {
    success: true,
    message: `Alias ${alias} now points to the selected version.`,
  };
};
...
export const deployRegistryVersion = async (
  versionId: string,
): Promise<RegistryVersion> => {
  const build = (await listBuilds()).find(
    (item) => item.version_id === versionId && item.status === "ready",
  );
