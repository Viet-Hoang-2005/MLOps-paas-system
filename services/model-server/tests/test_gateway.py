...
async def test_get_public_key_fetches_and_caches(monkeypatch):
    ...
    monkeypatch.setattr(
        index.httpx,
        "AsyncClient",
        lambda **kw: FakeAsyncClient(get=FakeResponse(payload={"keys": [key_data]})),
    )
...
async def test_predict_proxy_success_and_background_event(monkeypatch):
    ...
    response = await index.predict(
        "v",
        Mock(),
        index.InferenceRequest(features={"x": 1}),
        tasks,
        {"model_record": record},
    )
...
@pytest.mark.asyncio
async def test_predict_network_error_evicts_cache_and_returns_409_if_stopped(
    monkeypatch,
):
    record = {
        "id": "v-stopped",
