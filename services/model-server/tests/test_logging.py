...
def test_publication_summarizes_enqueue_failure_and_recovery_without_payload(
    monkeypatch,
):
    summary = Mock()
    producer = Mock()
    producer.produce.side_effect = [
...
async def test_jwks_recovery_waits_until_key_parsing_succeeds(monkeypatch):
    ...
    monkeypatch.setattr(
        ...
        Mock(
            side_effect=[
                ValueError("private-key-payload"),
                ValueError("private-key-payload"),
                "public-key",
            ]
        ),
...
@pytest.mark.asyncio
async def test_gateway_shutdown_closes_summaries_even_if_producer_flush_fails(
    monkeypatch,
):
...
async def test_prediction_uses_one_validated_id_and_trusted_context(
    ...
    result = await index.predict(
        "untrusted-route-id",
        request,
        index.InferenceRequest(features={"tenant_id": "untrusted-feature"}),
        tasks,
        {"model_record": record},
    )
...
async def test_prediction_resets_context_when_route_resolution_fails(monkeypatch):
    ...
    with pytest.raises(HTTPException):
        await index.predict(
            ...
            BackgroundTasks(),
            {"model_record": {"id": "v", "tenant_id": "t", "project_id": "p"}},
        )
    assert current_context() == before
...
def test_http_failure_keeps_resource_context_after_handler_reset(monkeypatch, caplog):
    ...
    app.dependency_overrides[index.verify_model_access] = lambda: {
        "model_record": {
            "id": "trusted-version",
            "tenant_id": "trusted-tenant",
            "project_id": "trusted-project",
    ...
    with TestClient(app) as client:
        response = client.post(
            "/models/request-version/predict",
            json={"features": {"project_id": "untrusted"}},
