...
from src.logging_utils import RequestLoggingMiddleware, bind_context, reset_context
...
def test_bentoml_uses_local_middleware_and_disables_duplicate_access_logs():
    ...
    ) in service.middlewares


def test_worker_reconfigures_logging_and_load_failure_omits_sensitive_details(
    monkeypatch,
):
    ...
    monkeypatch.setattr(
        index,
        "load_runtime_model",
        Mock(side_effect=RuntimeError("https://private/?token=hidden feature-payload")),
    )
    service = index.DeepLearningModelService()
...
def test_load_start_does_not_log_model_path(monkeypatch):
    ...
    monkeypatch.setattr(index, "log_event", event)
    monkeypatch.setattr(
        index, "download_model_artifact", lambda *_: "private-artifact-path"
    )
    ...
    assert (
        index.load_runtime_model("version", "https://private/?token=hidden") == "model"
    )
    event.assert_called_once_with(
        index.logger, "INFO", "model_load_started", "Loading deep learning model"
    )
...
def test_bentoml_http_failure_is_logged_without_framework_duplicate(
    monkeypatch, caplog
):
    ...
    with TestClient(index.DeepLearningModelService.to_asgi()) as client:
        for _ in range(2):
            response = client.post(
                "/predict", json={"payload": {"features": {"x": "feature-payload"}}}
            )
