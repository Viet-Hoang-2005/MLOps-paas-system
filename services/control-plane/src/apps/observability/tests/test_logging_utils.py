...
class LoggingTests(unittest.TestCase):
    ...
    def test_overflow_does_not_hide_server_errors_behind_warnings(self):
        ...
        try:
            ...
            with patch("common.logging_utils.time.monotonic", return_value=time.monotonic() + 120):
    ...
    def test_log_format_accepts_only_console_or_json(self):
        with patch.dict(os.environ, {"LOG_FORMAT": "json"}):
            ...
            self.assertIsInstance(formatter_for("test"), JsonFormatter)
        with patch.dict(os.environ, {"LOG_FORMAT": "unexpected"}):
            self.assertEqual(log_format(), "console")
            self.assertIsInstance(formatter_for("test"), ConsoleFormatter)
