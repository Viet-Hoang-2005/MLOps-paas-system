...
def decontaminate_training_raw(train: RawDataset, eval_hashes: set[str]) -> RawDataset:
    keep_mask = np.array(
        [row_hash not in eval_hashes for row_hash in train.row_hash_list], dtype=bool
    )
    ...
    clean_hashes = [
        row_hash for row_hash, keep in zip(train.row_hash_list, keep_mask) if keep
    ]
    return RawDataset(
        key=train.key,
        window=train.window,
...
def train_model(X: pd.DataFrame, y: np.ndarray, seed: int, name: str) -> XGBClassifier:
    ...
    if counts[0] == 0 or counts[1] == 0:
        raise ValueError(
            f"{name} training data must contain both BENIGN and ATTACK after binary mapping"
        )
...
def evaluate_model(
    ...
    return {
        ...
        "Macro-F1": float(
            f1_score(dataset.y, y_pred, average="macro", zero_division=0)
        ),
        "Weighted-F1": float(
            f1_score(dataset.y, y_pred, average="weighted", zero_division=0)
        ),
        "per_class": {
            label: {
...
def inspect_existing_models(root: Path, expected_feature_count: int) -> Dict[str, Any]:
    ...
    for name, paths in candidates.items():
        ...
        normalized = (
            [normalize_label(label) for label in label_classes] if label_classes else []
        )
        binary_projection_possible = bool(label_classes) and set(normalized).issubset(
            {"BENIGN", "ATTACK"}
        )
...
def leakage_checks(
    ...
    for train_name, train_ds in training_sets.items():
        for eval_name, eval_ds in eval_sets.items():
            ...
            overlapping_train_rows = sum(
                1 for row_hash in train_ds.row_hash_list if row_hash in eval_hashes
            )
            overlapping_eval_rows = sum(
                1 for row_hash in eval_ds.row_hash_list if row_hash in train_hashes
            )
            ...
            checks[f"{train_name}_vs_{eval_name}"] = {
                ...
                "overlap_ratio_eval": float(
                    overlapping_eval_rows / eval_ds.sampled_rows
                )
                if eval_ds.sampled_rows
                else 0.0,
            }
    return {
...
def make_readme(
    ...
    files_used = "\n".join(
        f"- `{info['path']}` ({info['role']})" for info in summaries.values()
    )
    leakage_line = (
...
def main() -> int:
    ...
    specs = [
        DatasetSpec(
            "train_m0",
            "TRAIN-M0",
            root / "data" / "train_2_classes.csv",
            "fixed baseline training",
            args.max_train_rows,
        ),
        DatasetSpec(
            "train_m1",
            "TRAIN-M1",
            root / "data" / "train_5_classes.csv",
            "challenger training",
            args.max_train_rows,
        ),
        DatasetSpec(
        ...
        DatasetSpec(
            ...
            "similar/holdout replay window",
            args.max_window_rows,
        ),
        DatasetSpec(
            "w2",
            "W2",
            root / "data" / "drift_portscan.csv",
            "drift replay window",
            args.max_window_rows,
        ),
        DatasetSpec(
            "w3",
            "W3",
            root / "data" / "drift_bruteforce.csv",
            "drift replay window",
            args.max_window_rows,
        ),
        DatasetSpec(
            "w4",
            "W4",
            root / "data" / "drift_webattacks.csv",
            "drift replay window",
            args.max_window_rows,
        ),
        DatasetSpec(
            "w5",
            "W5",
            root / "data" / "portscan.csv",
            "optional drift replay window",
            args.max_window_rows,
        ),
    ]
    ...
    raw_datasets["train_m0"] = decontaminate_training_raw(
        raw_datasets["train_m0"], eval_hashes
    )
    raw_datasets["train_m1"] = decontaminate_training_raw(
        raw_datasets["train_m1"], eval_hashes
    )
    ...
    model_m0 = train_model(
        datasets["train_m0"].X, datasets["train_m0"].y, seed, "M0-fixed-XGB"
    )
    ...
    for key in eval_keys:
        ...
        if drift["dataset_drift"]:
            if not first_drift_seen:
                ...
                action = (
                    "drift alert observed; review-driven retrained model already active"
                )
        else:
    ...
    fixed_metrics = {
        key: evaluate_model(model_m0, datasets[key], "M0-fixed-XGB")
        for key in eval_keys
    }
    challenger_metrics = {
        key: evaluate_model(model_m1, datasets[key], "M1-challenger-XGB")
        for key in eval_keys
    }
    ...
    for key in eval_keys:
        framework_active_metrics[key] = (
            challenger_metrics[key] if retrained_model_active else fixed_metrics[key]
        )
        ...
        drift_window_rows.append(
            {
                ...
                "BENIGN count": benign,
                "ATTACK count": attack,
                "Attack ratio": float(attack / ds.sampled_rows)
                if ds.sampled_rows
                else 0.0,
                "Drifted features": drift["drifted_feature_count"],
                "Drift share": drift["drift_share"],
    ...
    drift_method = (
        next(iter(drift_results.values()))["method"]
        if drift_results
        else "not_computed"
    )
    activated = comparison_df[
        comparison_df["Framework active model"] == "M1-challenger-XGB"
    ]
    f1_recovery_summary = {
        "delta_macro_f1_by_window": {
            str(row["Window"]): float(row["Delta Macro-F1"])
            for _, row in comparison_df.iterrows()
        },
        "mean_delta_macro_f1_after_retrained_model_active": float(
            activated["Delta Macro-F1"].mean()
        )
    ...
    fixed_df.to_csv(
        out_dir / "fixed_model_metrics.csv", index=False, float_format="%.6f"
    )
    challenger_df.to_csv(
        out_dir / "challenger_model_metrics.csv", index=False, float_format="%.6f"
    )
    comparison_df.to_csv(
        out_dir / "fixed_vs_framework_comparison.csv", index=False, float_format="%.6f"
    )
    drift_windows_df.to_csv(
        out_dir / "drift_windows.csv", index=False, float_format="%.6f"
    )
    per_class_df.to_csv(
        out_dir / "per_class_metrics.csv", index=False, float_format="%.6f"
    )
    ...
    (out_dir / "README_quantitative_results.md").write_text(
        make_readme(command, args, summaries, drift_method, leakage, comparison_df),
        encoding="utf-8",
    )
