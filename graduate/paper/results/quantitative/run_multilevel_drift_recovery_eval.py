...
def norm_label(v):
    return "BENIGN" if str(v).strip() in BENIGN_ALIASES else "ATTACK"
...
def shuffled(df, seed):
    return (
        df.sample(frac=1.0, random_state=seed).reset_index(drop=True)
        if not df.empty
        else df.copy()
    )
...
def compute_drift(ref_X, cur_X, pv_thr, drift_thr):
    ...
    for col in ref_X.columns:
        rv = ref_X[col].to_numpy(dtype=float)
        rv = rv[np.isfinite(rv)]
        cv = cur_X[col].to_numpy(dtype=float)
        ...
        if ks_2samp:
            if rv.size == 0 or cv.size == 0:
                ...
                col_drift = bool(r.pvalue < pv_thr)
                ks_stats.append(float(r.statistic))
        else:
            # PSI fallback
            edges = (
                np.unique(np.quantile(rv, np.linspace(0, 1, 11)))
                if rv.size
                else np.array([0, 1])
            )
            if edges.size < 2:
                ...
                cc, _ = np.histogram(cv, bins=edges)
                rp = np.maximum(rc / max(rc.sum(), 1), 1e-6)
                cp = np.maximum(cc / max(cc.sum(), 1), 1e-6)
                score = float(np.sum((cp - rp) * np.log(cp / rp)))
            col_drift = score >= 0.2
            ks_stats.append(score)
        if col_drift:
            drifted += 1
    ...
    return {
        ...
        "dataset_drift": bool(ds >= drift_thr),
        "method": method,
    }
...
def main():
    ...
    print("=" * 70)
    print("Multi-Level Drift Recovery Evaluation")
    print("=" * 70)
    ...
    for sid, family, path in attack_sources:
        ...
        all_data[sid] = {
            "family": family,
            "trigger_atk": trigger_atk,
            "future_atk": future_atk,
        }
    ...
    for sid, family, path in attack_sources:
        ...
        future_atk = d["future_atk"]

        m1_df = decontam(
            pd.concat([train_clean, trigger_atk], ignore_index=True),
            cols,
            set(row_hashes(future_atk, cols)),
        )
        print(f"\n[{sid} {family}] Training M1 on {len(m1_df):,} rows ...")
        model_m1 = train_xgb(
            prep_X(m1_df, cols, medians), prep_y(m1_df), seed + hash(sid) % 100
        )
        ...
        for level_name, atk_frac in DRIFT_LEVELS:
            ...
            n_ben = (
                0
                if atk_frac >= 1.0
                else min(int(n_atk * (1 - atk_frac) / atk_frac), len(benign_pool))
            )

            atk_s = future_atk.sample(
                n=n_atk, random_state=seed + 1, replace=False
            ).reset_index(drop=True)
            if n_ben > 0:
                ben_s = benign_pool.sample(
                    n=n_ben, random_state=seed + 2, replace=False
                ).reset_index(drop=True)
                win_df = shuffled(
                    pd.concat([atk_s, ben_s], ignore_index=True), seed + 3
                )
            else:
                win_df = atk_s

            y_win = prep_y(win_df)
            actual_atk = int((y_win == 1).sum())
            actual_ben = int((y_win == 0).sum())
            actual_pct = (
                actual_atk / (actual_atk + actual_ben)
                if (actual_atk + actual_ben)
                else 0.0
            )

            win_X = prep_X(win_df, cols, medians)
            drift = compute_drift(
                ref_X_drift, win_X, args.pvalue_threshold, args.drift_threshold
            )
            ...
            rows.append(
                {
                    ...
                    "BENIGN rows": actual_ben,
                    "ATTACK rows": actual_atk,
            ...
            print(
                f"  [{level_name:8s} {actual_pct * 100:4.0f}%atk] "
    ...
    df.to_csv(
        output / "multilevel_drift_recovery_comparison.csv",
        index=False,
        float_format="%.6f",
    )
    ...
    df[paper_cols].to_csv(
        output / "multilevel_paper_table.csv", index=False, float_format="%.4f"
    )

    write_json(
        output / "multilevel_experiment_config.json",
        {
            "created_at_utc": datetime.now(timezone.utc).isoformat(),
            "command": "python " + " ".join(sys.argv),
            "random_seed": seed,
            "drift_levels": [
                {"name": n, "attack_fraction": f} for n, f in DRIFT_LEVELS
            ],
